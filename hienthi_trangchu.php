...
<body>
	...
	<div class="container">
		...
		<div class="content">	<!--  content -->
			...
			<div class="header">
				<h4 style="padding-left: 15px;">LAPTOP <?php echo $tenHang; ?></h4>
				<form action="" name="frm_SapXep" method="post"> 
					<span>Sắp xếp theo: </span><select name="sapXep" onChange="frm_SapXep.submit();">
						<option value="tangdan" <?php echo $chooseASC; ?>>Giá thấp đến cao</option>
						<option value="giamdan" <?php echo $chooseDESC; ?>>Giá cao đến thấp</option>
					</select>
				</form>
			</div>
			...
			<div class="sub_content">
				<?php 
						...
						$luot = 0;
						if($sapXep == "giamdan"){
							$showSanPham='SELECT id_SanPham, hinhAnh, tenSanpham, gia, manHinh, CPU, Ram, VGA, HDH, nang FROM sanpham WHERE hang="'.$tenHang.'" ORDER BY gia DESC';
						}else{
							$showSanPham='SELECT id_SanPham, hinhAnh, tenSanpham, gia, manHinh, CPU, Ram, VGA, HDH, nang FROM sanpham WHERE hang="'.$tenHang.'" ORDER BY gia ASC';
						}
						if($tenHang == "All"){
							$showSanPham='SELECT id_SanPham, hinhAnh, tenSanpham, gia, manHinh, CPU, Ram, VGA, HDH, nang FROM sanpham ORDER BY gia '.$sortALL;
						}
